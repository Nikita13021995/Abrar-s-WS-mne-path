...
for index in index_array_2[:]:  
    i            = index
    src          = mne.setup_source_space(sub_list[index], spacing='oct6', 
                                          subjects_dir = data_path,  n_jobs=4) #add_dist='patch', 
    print(src)
    ...
    mne.write_source_spaces('Sub{}-oct6-src.fif'.format(index), src, overwrite = True)
...
for index in index_array_2[:]:  
    ...
    vol_src     = mne.setup_volume_source_space(subject, pos = 5,  mri = fname_aseg,
    ...
    mne.write_source_spaces('Sub{}-vol-hypoamyg_pos5-src.fif'.format(index), vol_src, overwrite = True) 
...
for index in index_array_2[:]:  
    ...
    mne.write_source_spaces('Sub{}-oct6-mixed-hypoamyg-src.fif'.format(index), src_mixed, overwrite = True)
        
#### FSAVERAGE SOURCE SPACE
f_avg_path              = 'L:/SATURN/ABRAR WM/fif for coreg _ forward modeling/subjects'
src_avg                 = mne.setup_source_space('fsaverage', spacing = 'oct6',  subjects_dir = f_avg_path)
src_avg_vol             = mne.setup_volume_source_space('fsaverage', pos = 5,    subjects_dir = f_avg_path,  
...
mne.write_source_spaces('Sub_avg-vol-hypoamyg_pos5-src.fif', src_avg_vol, overwrite = True)
mne.write_source_spaces('Sub_avg-mix-hypoamyg_pos5-src.fif', src_avg_mix, overwrite = True)
...
for index in index_array[:]: 
    ...
    src_surf            = mne.read_source_spaces('Sub{}-oct6-src.fif'.format(index))
    os.chdir('L:/SATURN/ABRAR WM/FEB/MIXED SOURCE SPACE _ dec') 
    src_mix             = mne.read_source_spaces('Sub{}-oct6-mixed-hypoamyg-src.fif'.format(index))
    ...
    #SAVER
    os.chdir('L:/SATURN/ABRAR WM/FEB') 
    mne.write_forward_solution('Sub{}-oct6-dec-surf-fwd.fif'.format(index), fwd, 
                               overwrite = True) 
    ...
    #SAVER
    os.chdir('L:/SATURN/ABRAR WM/FEB') 
    mne.write_forward_solution('Sub{}-oct6-dec-mix-fwd.fif'.format(index), fwd_m, 
                                overwrite = True) 
...
os.chdir('L:/SATURN/ABRAR WM/FEB/SURFACE SOURCE SPACE _ dec') 
src_avg                 = mne.read_source_spaces('Sub_avg-oct6-src.fif')
os.chdir('L:/SATURN/ABRAR WM/FEB/MIXED SOURCE SPACE _ dec')
src_avg_mix             = mne.read_source_spaces('Sub_avg-mix-hypoamyg_pos5-src.fif')
conductivity            = (0.3,)
...
mne.write_bem_solution(    'Sub_ave-bem-sol.fif',           bem,   overwrite = True)
mne.write_forward_solution('Sub_ave-oct6-dec-surf-fwd.fif', fwd,   overwrite = True) 
mne.write_forward_solution('Sub_ave-oct6-dec-mix-fwd.fif',  fwd_m, overwrite = True) 
...
for index in index_array_2[:1]:    
    ...
    #DICS CREATION
    # depth            = mne.forward.compute_depth_prior(fwd_ind[index], info, 
    #                   exp=0.8, limit=10.0, limit_depth_chs=False, 
    #                   combine_xyz='spectral', noise_cov=None, rank=None, verbose=None)

    dics_filter_s     = mne.beamformer.make_dics(info,fwd, csd_s, reg=0.05, 
                                                      pick_ori='max-power',
                                                      inversion='single', 
                                                      weight_norm=None, 
                                                      real_filter=True, depth = 1.) 
    dics_filter_t     = mne.beamformer.make_dics(info,fwd, csd_t, reg=0.05,
                                                      pick_ori='max-power',
                                                      inversion='single',
                                                      weight_norm=None, 
                                                      real_filter=True,  depth = 1.) 
    dics_filter_Ab    = mne.beamformer.make_dics(info,fwd, csd_Ab, reg=0.05, 
                                                      pick_ori='max-power',
                                                      inversion='single',
                                                      weight_norm=None,
                                                      real_filter=True,  depth = 1.) 
    ...
    type( dics_filter_t)
    os.chdir('L:/SATURN/ABRAR WM/JAN') 
    dics_filter_s.save('Sub{}_dec_from_{}_to_{}_{}_surf_spatial-dics.h5'.format( index,a,b,orientation), overwrite = True) 
    dics_filter_t.save('Sub{}_dec_from_{}_to_{}_{}_surf_temporal-dics.h5'.format(index,a,b,orientation), overwrite = True) 
    dics_filter_Ab.save('Sub{}_dec_from_{}_to_{}_{}_surf_AvBase-dics.h5'.format( index,a,b,orientation), overwrite = True)
    
    #SOURCE ESTIMATE - FILTER APPLICATION
    stc_s,  freq_s    = mne.beamformer.apply_dics_csd(csd_s,  dics_filter_s )
    stc_t,  freq_t    = mne.beamformer.apply_dics_csd(csd_t,  dics_filter_t )
    stc_Ab, freq_Ab   = mne.beamformer.apply_dics_csd(csd_Ab, dics_filter_Ab)
    print(stc_s)
    type( stc_s)
    ...
    #SAVER
    os.chdir('L:/SATURN/ABRAR WM/JAN') 
    stc_s.save( 'Sub{}_dec_from_{}_to_{}_{}_surf_spatial_stc'.format( index,a,b,orientation),overwrite=True) 
    stc_t.save( 'Sub{}_dec_from_{}_to_{}_{}_surf_temporal_stc'.format(index,a,b,orientation),overwrite=True) 
    stc_Ab.save('Sub{}_dec_from_{}_to_{}_{}_surf_AvBase_stc'.format(  index,a,b,orientation),overwrite=True)
...
for index in index_array_2[:1]:    
    ...
    type(dics_filter_t)
    os.chdir('L:/SATURN/ABRAR WM/JAN') 
    dics_filter_s.save( 'Sub{}_dec_from_{}_to_{}_{}_mix_spatial-dics.h5'.format( index,a,b,orientation), overwrite = True) 
    dics_filter_t.save( 'Sub{}_dec_from_{}_to_{}_{}_mix_temporal-dics.h5'.format(index,a,b,orientation), overwrite = True) 
    dics_filter_Ab.save('Sub{}_dec_from_{}_to_{}_{}_mix_AvBase-dics.h5'.format(  index,a,b,orientation), overwrite = True)
    ...
    #SAVER
    os.chdir('L:/SATURN/ABRAR WM/JAN') 
    stc_s.save( 'Sub{}_dec_from_{}_to_{}_{}_mix_spatial_stc'.format( index,a,b,orientation),overwrite=True) 
    stc_t.save( 'Sub{}_dec_from_{}_to_{}_{}_mix_temporal_stc'.format(index,a,b,orientation),overwrite=True) 
    stc_Ab.save('Sub{}_dec_from_{}_to_{}_{}_mix_AvBase_stc'.format(  index,a,b,orientation),overwrite=True)
...
i                    = 3
for i in index_array_2[:1]:    
    index            = i
    os.chdir('L:/SATURN/ABRAR WM/JAN') 
    #SURFACE STC
    stc_s_surf[i]    = mne.read_source_estimate('Sub{}_dec_from_{}_to_{}_{}_surf_spatial_stc'.format( i,a,b, orientation))
    stc_t_surf[i]    = mne.read_source_estimate('Sub{}_dec_from_{}_to_{}_{}_surf_temporal_stc'.format(i,a,b, orientation))
    stc_A_surf[i]    = mne.read_source_estimate('Sub{}_dec_from_{}_to_{}_{}_surf_AvBase_stc'.format(  i,a,b, orientation))
    #MIXED STC
...
for i in index_array_2[:1]: 
    ...
    # brain_before_2   = stc_before.plot(subject=sub_list[i], surface='pial', 
    #                                     hemi='both',         colormap='auto', 
    #                                     time_label='auto',   smoothing_steps=10, 
    #                                     transparent=True,    alpha=.8, time_viewer='auto', 
    #                                     subjects_dir=f_avg_path,    figure=None, 
    #                                     views=['dorsal', 'lateral', 'medial','ventral'], 
    #                                     colorbar=True, clim='auto', cortex='classic', 
    #                                     size=800, background='black', 
    ...
    #                                     brain_kwargs=None, verbose=None)
    os.chdir('L:/SATURN/ABRAR WM/JAN') 
    # brain_before.save_image('Sub{}_from{}_to_{}_cont_before.png'.format(i,a,b))
    brain_before.save_image('PLAYING_BEFORE.png'.format(i,a,b))

    # brain_before_2.save_image('Sub{}_from{}_to_{}_cont_before_2.png'.format(i,a,b))
    ...
    stc_fs_s.save('Sub{}_from_{}_to_{}_{}_morph_surf_spatial_stc'.format(   i,a,b,orientation),overwrite=True)
    stc_fs_t.save('Sub{}_from_{}_to_{}_{}_morph_surf_temporal_stc'.format(  i,a,b,orientation),overwrite=True) 
    stc_fs_A.save('Sub{}_from_{}_to_{}_{}_morph_surf_AvBaseline_stc'.format(i,a,b,orientation),overwrite=True)
    morph_surf_s.save('Sub{}_from_{}_to_{}_{}_morph_surf_s.h5'.format(      i,a,b,orientation),overwrite=True)
    morph_surf_t.save('Sub{}_from_{}_to_{}_{}_morph_surf_t.h5'.format(      i,a,b,orientation),overwrite=True)
    morph_surf_A.save('Sub{}_from_{}_to_{}_{}_morph_surf_A.h5'.format(      i,a,b,orientation),overwrite=True)
    ...
    brain_after     = stc_after.plot(subject='fsaverage', surface='inflated', hemi='both', 
                      ...
                      volume_options=1.0, view_layout='vertical', add_data_kwargs=None,
                      brain_kwargs=None, verbose=None)
    brain_after.save_image('PLAYING_AFTER_SPACING.png'.format(i,a,b))
    # brain_after_2     = stc_after.plot(subject='fsaverage', surface='inflated', hemi='both', 
    #                   colormap='auto', time_label='auto', smoothing_steps=10,
    #                   transparent=True, alpha=1.0, time_viewer='auto', subjects_dir=f_avg_path, 
    #                   figure=None, views=['dorsal', 'lateral', 'medial','ventral'], colorbar=True, clim='auto', cortex='classic',
    #                   size=800, background='black', foreground=None, initial_time=0, time_unit='s',
    ...
    #                   brain_kwargs=None, verbose=None)
    # os.chdir('L:/SATURN/ABRAR WM/JAN') 
    # brain_after.save_image('Sub{}_from{}_to_{}_cont_{}_after.png'.format(i,a,b,orientation))
    # brain_after_2.save_image('Sub{}_from{}_to_{}_cont_{}_after_2.png'.format(i,a,b,orientation))
    # stc_after.save('Sub{}_from_{}_to_{}_surf_cont_{}_stc'.format(i,a,b,orientation), overwrite=True)
