...
pta                                    = list_t_ped_np # 26 for (204, 30)
...
T_obs_plot                            = 1 * np.ones_like(T_obs)
for c, p_val                          in zip(clusters, cluster_p_values):
    if p_val                          <= p_accept:
         T_obs_plot[c]                = T_obs[c]
         
plt.imshow(T_obs, aspect='auto', origin='lower', cmap='cividis', vmin=-3, vmax=3) #, extent=[1,204,4,80]) #freq not just linear, but as index
# plt.imshow(T_obs_plot, aspect='auto', origin='lower', cmap='cool', vmin=-3, vmax=3) #, extent=[1,204,4,80]) #freq not just linear, but as index
...
R               = T_sig_coef                                                          #T_obs_plot; T_sig_coef; S_sig_coef
epochs          = epochs_s_28
np.shape(R)
theta           = R[0:6][:]
alpha           = R[7:11][:]
...
im,cm           = mne.viz.plot_topomap(coef, epochs.info, cmap = 'Blues')#, vmin=vmax, vmax=vmin)        #Blues
clb             = fig.colorbar(im, orientation = 'horizontal',extend='both')
...
for i in index_array_2:    
    ...
    csd_s = mne.time_frequency.csd_morlet(epochs_s, frequencies, tmin=-8, tmax=8, n_cycles=5, decim=20, n_jobs=-1)    #!!!! CHECK
    csd_s.save('S{}_nov_full_S_csd.h5'.format(i), overwrite=True, verbose=None)
    csd_t = mne.time_frequency.csd_morlet(epochs_t, frequencies, tmin=-8, tmax=8, n_cycles=5, decim=20, n_jobs=-1)
